...
state={posts:[],coments:[]}
    ...
    fetchPost() {
        ...
        let posts = database.ref(`offTopic/${postName}`);
        posts.on('value', (snapshot) => {
            //this.setState({postName:snapshot.val()});
            this.setState({ post: snapshot.val() });
        })
        let coments = database.ref(`offTopic/${postName}/coments`)
    ...
    PostComent() {
        ...
        if (comentario.trim() !== "" && userA!==null) {
            ...
            let User = userA.displayName
            let _input = input.split(' ').join('_')
            let coments = { User, comentario };
            let db = firebase.database()
            db.ref(`offTopic/${_input}/coments`).set(coments)
